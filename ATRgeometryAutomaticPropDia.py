# -*- coding: utf-8 -*-
"""
Created on Thr Apr 12 11:25:08 2018

Geometry class for the ATR72
Includes evolution to automatic propeller diameter computation and positionning
For use with prop-wing interaction of Patterson and Jameson
...
#    Cn_beta_f=-0.15
    ...
    def __init__(self, VTsize, N_eng=2, inop_eng=0, bv=4.42, r=0.6, zw=1.8, rf=1.3, zh=3.71, bvl=5.91, Sh=11.13, Sv=12.5):
        ...
        if N_eng>2:
            ...
            """ Propeller diameter
            Takes into account the limitation of Patterson theory for propeller
            at wingtip. Thus leave one Prop Diam away from wingtip.
            """
            self.Dp = (self.b/2-self.FusWidth/2)/(N_eng/2+1)
            self.Sp = self.Dp**2/4*math.pi
            self.xp = self.Dp/2
            self.step_y=self.Dp
            self.PosiEng=np.append(np.arange(-self.b/2+self.Dp*3.0/2.0,-self.FusWidth/2,self.step_y),np.arange(self.FusWidth/2+self.Dp/2,self.b/2-self.Dp,self.step_y))
        else:
            self.step_y=8.1/2.0
