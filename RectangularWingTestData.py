...
#            self.Dp = (self.b/2-self.FusWidth/2)/(N_eng/2+1)
            self.Dp = (self.b/2-self.FusWidth/2-(N_eng/2-1)*dprop)/(N_eng/2+dfus+dtip)
            self.Sp = self.Dp**2/4*math.pi
            self.xp = self.Dp/2
            self.step_y=self.Dp+dprop
            self.PosiEng=np.append(np.arange(-self.b/2+self.Dp*(dtip+0.5),-self.FusWidth/2-self.Dp/2,self.step_y),np.arange(self.FusWidth/2+self.Dp*(dfus+0.5),self.b/2-self.Dp*dtip,self.step_y))
        else:
            self.step_y=8.1/2.0
